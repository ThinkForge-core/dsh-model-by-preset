...
- Developed and verified against **DSH `0.1.1-rc.2`**. Functionality on another
  version of DeepSeek Harness is **not guaranteed** (`install.py` warns when the
  installed `dsh --version` does not match the target).
...
### Option 0 — automated cross-platform installer (recommended for dev checkouts)
...
```bash
cd /path/to/dsh-model-by-preset
npm pack                      # -> dsh-model-by-preset-0.1.1.tgz
dsh plugin --profile web remove dsh-model-by-preset   # only if already installed
dsh plugin --profile web add file:/abs/path/to/dsh-model-by-preset-0.1.1.tgz
```
